...
Bool bitIsSet(Bits b, int position)
{
	...
	Byte mask = (1<<(position-index*8));
	if((b->bitstring[index]&mask)==mask) return TRUE;
	return FALSE;
}
...
void setBit(Bits b, int position)
{
	...
	int index= position/8;
	Byte mask = (1<<(position-index*8));
	b->bitstring[index]=(b->bitstring[index]|mask);
}
...
void unsetBit(Bits b, int position)
{
	...
	//TODO
          int index= position/8;
          Byte mask = (1<<(position-index*8));
          b->bitstring[index]=~b->bitstring[index];
          b->bitstring[index]=b->bitstring[index]|mask;
          b->bitstring[index]=~b->bitstring[index];
...
void shiftBits(Bits b, int n)
{
    ...
    Byte mask=0;
    Byte temp=0;
    int byte_shift = 0; // if n>8 then pass bitstring one by one
    int bit_shift = 0;  // the left bits need to be shift
    if(n>=0){
        byte_shift = n/8;
        bit_shift = n%8;
        if(byte_shift>0) {
            for (int i = b->nbytes - 1; i >= 0; i--) {
                if (i + byte_shift < b->nbytes){
                    b->bitstring[i+byte_shift] = b->bitstring[i];
                }
                b->bitstring[i]=0;
            }
        }
        ...
        for(int i=0;i<bit_shift;i++){
            for (int  j= 0; j <b->nbytes ; j++) {
                mask=temp>>7;
                temp = b->bitstring[j];
                b->bitstring[j]=b->bitstring[j]<<1;
                b->bitstring[j]=b->bitstring[j]|mask;
            }
        }
    }
    else{
